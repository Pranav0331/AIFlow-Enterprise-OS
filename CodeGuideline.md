...
```
project-root/
  ├── convex/                # Convex backend: schema, auth, queries/mutations (system of record)
  │   └── lib/               # Shared backend helpers (e.g. tenant scoping)
  ...
  │   │   ├── dashboard/     # Admin Dashboard shell (layout, nav config, placeholders)
  │   │   └── system/        # App-level system notices (e.g. Convex setup notice)
  │   ├── config/            # Plain constants such as the Convex deployment URL
  │   ├── hooks/             # Custom React hooks
  │   ├── i18n/              # i18n runtime: config.ts (entry) + util.ts (helpers)
  │   ├── lib/               # Utility functions and libraries (incl. Convex client/api)
  │   ├── pages/             # Application pages (each page in its own subdirectory)
  │   │   ├── landing/       # Public marketing landing page
  │   │   ├── signup/        # Company signup (creates tenant + admin)
  │   │   ├── login/         # Login
  │   │   └── dashboard/     # Protected Admin Dashboard routes (one subfolder per section)
...
> **Backend note:** This project uses Convex — not Enter Cloud — as its system of record (database, auth, real-time sync, serverless functions), deployed independently via the Convex CLI. The frontend connects directly to Convex through `convex/react` and `@convex-dev/auth/react` (see `src/lib/convex.ts`, `src/config/convex.ts`). Multi-tenant isolation is enforced in every `convex/*.ts` query/mutation by deriving `tenantId` from the authenticated caller (`convex/lib/tenant.ts`) — never from client input.
